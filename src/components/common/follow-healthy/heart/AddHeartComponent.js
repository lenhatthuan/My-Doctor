...
const AddHeartComponent = props => {
  ...
  const handleValueInputDiastole = text => {
    ...
    if (text > diastole) setMessageSystole('');
  ...
  const handleValueInputSystole = text => {
    ...
    if (text < systole) setMessageDiastole('');
  ...
  const onSave = () => {
    ...
    if (modalVisible) {
      AsyncStorage.getItem('patientData').then(res => {
        ...
        createHeart(patient.patientId, systole, diastole, heartBeat).then(
          heart => {
            if (heart) {
  ...
  return (
    <>
      <SbNotification
        ...
        message={'Đã thêm chỉ số nhịp tim thành công!'}
