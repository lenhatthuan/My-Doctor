...
import { balanceFormat, formatDate, addDays, discription,  getTime } from "../../utils/string-format";
import { RadioButton } from "react-native-paper";
import { getServiceById } from '../../store/actions/service';
import CountDown from "react-native-countdown-component";
import {deleteRegistration, getAllByPatientIdAndDoctorId, updateRegistration} from "../../store/actions/doctor-registration";
...
const ServiceDetail = props =>{
  ...
  useEffect(() => {
    ...
    const updateExpired = () => {
        updateRegistration(registration.id, registration.name, "EXPIRED").then((res) => {});
      };
    ...
    return (
        <View style = {styles.screen}>
         ...
         <View style = {{backgroundColor: '#FF7800', height: 2, width: 45, borderRadius: 10, marginTop: 10}}></View>
       ...
       >
         ...
         <Pressable style={{ marginTop: 5, flexDirection: 'row' }}>
           ...
           <Text style = {{padding: 5, fontWeight: 'bold'}}>{registration.duration}</Text>
         ...
         >
           ...
           <Text
             style={{ color: "#1A5F7A", fontWeight: "bold", marginLeft: 5, marginTop: 10, marginBottom: 10 , justifyContent: 'center', textAlign: 'center'}}
           >
       ...
       <LoadingTranComponent visible = {isLoading} message = "Delete..."/>
          ...
          {registration.status === "CREATED" || registration.status === "CANCEL" || registration.status === "EXPIRED"? <View style = {styles.btnDelete}><Text style = {styles.txtBtn} onPress={() => btnDeleteonPress()}>Xóa</Text></View>
            :null}

            {registration.status === "CREATED" || registration.status === "CANCEL" || registration.status === "EXPIRED"? <View style = {styles.btnDelete}><Text style = {styles.txtBtn} onPress={() => btnDeleteonPress()}>Xóa</Text></View>
            :null}
           
           {registration.status === "CREATED" ? <View style = {styles.btn}><Text style = {styles.txtBtn} onPress={() => gotoPayment()}>Tiếp tục thanh toán</Text></View>
            :null}

            {registration.status === "EXPIRED"  || registration.status === "CANCEL" ? <View style = {styles.btn}><Text style = {styles.txtBtn} onPress={() => gotoPayment()}>Tiếp tục đăng ký</Text></View>
            :null}
